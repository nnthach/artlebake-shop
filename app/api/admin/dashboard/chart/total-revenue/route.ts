...
import { formatLabelChart } from "@/utils/format-sth";
      ...
      const result = weekDates.map((date) => {
        ...
        const totalRevenue =
          orders?.reduce((sum, order) => {
            const orderDate = formatDateChart(
              new Date(
            ...
            return orderDate === dateString ? sum + Number(order.total) : sum;
          }, 0) ?? 0;

        return {
          dateLabel: formatLabelChart(date, "day"),
          totalRevenue,
        };
      });
